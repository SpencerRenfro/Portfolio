...
import { preloadImage, isImageCached, getCachedImage } from '../../utils/imagePreloader';
...
const PreloadedImage = ({
  ...
  useEffect(() => {
    ...
    if (!isImageCached(src)) {
      preloadImage(src)
        .then((img) => {
          if (img) {
            ...
            // Image failed to load but we resolved with null
            setError(true);
          }
        })
        .catch(() => {
      ...
      // Check if the cached image is null (failed to load)
